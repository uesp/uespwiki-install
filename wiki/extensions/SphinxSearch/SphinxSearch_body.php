	...
	function execute( $par ) {
		...
		if ( $cl ) {
			...
			if ( $res === false ) {
				...
				wfRunHooks( 'SpecialSearchResults', array( $term, &$res, &$res, $searchtime ) );
	...
	function createNewSearchForm( $term ) {
		...
		if ( $wgSphinxSearch_mode == SPH_MATCH_EXTENDED ) {
			$wgOut->addHTML( "<label><input type='radio' name='match_all' value='0' " .
				...
				wfMsg( 'sphinxMatchAny' )  . '</label>' .
				" <label><input type='radio' name='match_all' value='1' " .
				( $wgSphinxMatchAll ? "checked='checked'" : "" ) . " />" .
				wfMsg( 'sphinxMatchAll' ) . '</label>'
			);
		}
		$wgOut->addHTML( " &#160; <label><input type='checkbox' name='match_titles' value='1' " .
			( $wgRequest->getInt( 'match_titles' ) ? "checked='checked'" : "" ) . ">" .
			wfMsg( 'sphinxMatchTitles' ) . "</label></div>"
		);
