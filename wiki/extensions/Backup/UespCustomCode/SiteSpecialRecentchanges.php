...
class SiteSpecialRecentChanges extends SpecialRecentChanges
{
	...
	function optionsPanel($defaults, $nondefaults, $numRows)
	{
		...
		$minorLink = $this->makeOptionsLink(
			$showhide[1 - $options['hideminor']],
			array('hideminor' => 1 - $options['hideminor']),
			$nondefaults
		);
		$botLink = $this->makeOptionsLink(
			$showhide[1 - $options['hidebots']],
			array('hidebots' => 1 - $options['hidebots']),
			$nondefaults
		);
		$anonsLink = $this->makeOptionsLink(
			$showhide[1 - $options['hideanons']],
			array('hideanons' => 1 - $options['hideanons']),
			$nondefaults
		);
		$liuLink   = $this->makeOptionsLink(
			$showhide[1 - $options['hideliu']],
			array('hideliu' => 1 - $options['hideliu']),
			$nondefaults
		);
		$patrLink  = $this->makeOptionsLink(
			$showhide[1 - $options['hidepatrolled']],
		...
		$userspaceLink = $this->makeOptionsLink(
			$showhide[1 - $options['hideuserspace']],
			array('hideuserspace' => 1 - $options['hideuserspace']),
			$nondefaults
		);
		if ($options['usecustomns'])
			...
			$text = 'Turn on';
		$this->customnslink = $this->makeOptionsLink(
			$text,
			array('usecustomns' => 1 - $options['usecustomns'], 'namespace' => ''),
			$nondefaults
		);
